...
// The §21 KNOWN_REFERENCES and §27 KNOWN_MATCHES tables live in the sibling
// data modules `fixture-ground-truth-references.ts` /
// `fixture-ground-truth-matches.ts` (keeps every file < 400 lines);
// `KnownReferenceEntry` / `KnownMatchRow` / `KnownEntryStatusRow` /
// `KnownMatchMap` stay here so all modules share one type.

// ---------------------------------------------------------------------------
// S04-T4 — §27 KNOWN_MATCHES ground-truth row types (the table itself lives
// in the sibling data module `fixture-ground-truth-matches.ts`).
// ---------------------------------------------------------------------------
